...
if True:
    ...
    model = SeveralAlgorithms([
        VMPO('MlpPolicy', env, batch_size=1024, policy_kwargs=dict(optimizer_class=torch.optim.SGD, optimizer_kwargs=dict(weight_decay=1e-3)), learning_rate=1e-3, gamma=0.99, verbose=1)
        for j in range(6)
    ], env)
    # Train the agent
