...
if True:
    ...
    model = SeveralAlgorithms([
        PPO('MlpPolicy', env, batch_size=128, learning_rate=1e-3, gamma=0.9, verbose=1)
        for j in range(6)
    ], env)
    # Train the agent
    import codeit
    codeit.codeit(l=locals())
    model.learn(callback=RecorderCallback("recording.txt"), total_timesteps=int(4000000))
    # Save the agent
