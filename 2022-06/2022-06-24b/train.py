...
class alternator(pseudo):
    def __init__(self, env):
        super().__init__(env)
        self.last = 2
    def decide(self, opponent):
        if opponent == 2 or opponent == self.last:
            out = numpy.random.randint(2)
        else:
            out = opponent
        self.last = out
        return out
...
if True:
    ...
    model = SeveralAlgorithms([alternator(env) for _ in range(5)] + [
        VMPO('MlpPolicy', env, batch_size=128, policy_kwargs=dict(optimizer_class=torch.optim.SGD, optimizer_kwargs=dict(weight_decay=1e-4)), learning_rate=1e-3, gamma=0.99, verbose=1)
        for j in range(1)
    ], env)
    # Train the agent
