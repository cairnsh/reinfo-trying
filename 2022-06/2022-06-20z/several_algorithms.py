...
class SeveralAlgorithms:
    ...
    def predict(self, observation, state = None, episode_start = None, deterministic = False):
        ...
        def predict_single(ix):
            return policies[ix].predict(
                SLICE_TENSOR(observation, ix),
                deterministic=deterministic
            )

        predictions = [predict_single(ix) for ix in range(len(policies))]
    ...
    def collect_rollouts(
        ...
        want_the_distribution = [getattr(_, "want_the_distribution", False) for _ in rollout_buffer_list]
        ...
        while n_steps < n_rollout_steps:
            ...
            for algorithm_ix in range(n_algorithms):
                with th.no_grad():
                    ...
                    if want_the_distribution[algorithm_ix]:
                        dist = GET_THE_SINGLE_ITEM(policies[algorithm_ix].get_distribution(obs_tensor).distribution.logits.cpu().numpy())
            ...
            # actions might be an ndarray with shape (1, ...)
            # or a list with one entry. in either case, we get
            # the single item from each action and put them in a list
            flattened_clipped_action_list = [GET_THE_SINGLE_ITEM(actions) for actions in clipped_action_list]
            ...
            # just hack in a recording callback
            f = getattr(callback, "special_record_games")
            if f is not None:
                f(self._last_obs[:, 0], flattened_clipped_action_list)
            ...
            for algorithm_ix in range(n_algorithms):
                ...
                if want_the_distribution[algorithm_ix]:
                    dist = dist_list[algorithm_ix][None, :]
                
                reward = SLICE_LIST(rewards, algorithm_ix)
                last_done = SLICE_LIST(self._last_episode_starts, algorithm_ix)
                args = last_obs, action, reward, last_done, value, log_prob
                if want_the_distribution[algorithm_ix]:
                    rollout_buffer_list[algorithm_ix].add_with_distribution(*args, dist)
                else:
                    rollout_buffer_list[algorithm_ix].add(*args)
        ...
        f = getattr(callback, "on_rollout_end", None)
        if f is not None:
            f()
    ...
    def learn(
        ...
        while VALUE_OF_LIST_WHICH_IS_ALL_THE_SAME_VALUE([algorithm.num_timesteps for algorithm in self.algorithmlist]) < total_timesteps:
            ...
            after_training_epoch = getattr(callback, "after_training_epoch", None)
            if after_training_epoch is not None:
                after_training_epoch(self, self.algorithmlist[0].num_timesteps)
        return self
