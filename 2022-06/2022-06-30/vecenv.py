...
class MatrixGameVecEnv(VecEnv):
    def __init__(self, n_actions, n_players, matrix):
        ...
        self.matchlog = open("matching.log", "w")
    ...
    def _reset_observations_using_matching(self):
        for i in range(self.n_players):
            ...
            observation[0:] = self.empty_history_entry
    ...
    def step(self, action):
        ...
        if self.time == EPISODELENGTH:
            ...
            done = [True] * self.n_players
            self.reset()
        else:
            info = [{} for _ in range(self.n_players)]
            done = [False] * self.n_players

        return np.stack(self.observations_per_player), np.array(reward), np.array(done), info
